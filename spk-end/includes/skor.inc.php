...
class Skor
{
	...
	function insert($a, $b, $c, $d)
	{
		...
		$query = "replace into " . $this->table_name . " values('$a','$b',0,'$c','$d')";
	...
	function insert3($a, $b, $c)
	{
		...
		$query = "replace into ahp_jum_alt_kri values('$a','$b','$c',0,0)";
		$stmt = $this->conn->prepare($query);
	...
	function delete_ahp_jum_alt_kri(){
		$query = $query = "DELETE FROM ahp_jum_alt_kri";
		$stmt = $this->conn->prepare($query);
		if ($result = $stmt->execute()) {
			return true;
		} else {
